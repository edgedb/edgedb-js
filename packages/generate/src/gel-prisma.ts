...
// const GEL_DSN = 'edgedb://edgedb@localhost:5656/main?tls_security=insecure'
// const client = edgedb.createClient(GEL_DSN)
...
function warn(msg: string) {
  // Keep record of warnigns during the generation process
  WARNINGS.push(msg);
}
...
function validName(name: string): boolean {
  ...
  if (!CLEAN_NAME.test(name)) {
    warn(
      `Non-alphanumeric names are not supported: ${ name }`)
    return false
  }
  return true
}
  ...
  for (const spec of types) {
    ...
    for (const spec of types) {
      ...
      for (const link of spec.backlinks) {
        if (link.name.search(/^backlink_via_/) >= 0) {
          if (!bk[link.name]) {
            bk[link.name] = []
          }
...
class ModelClass {
  ...
  getBacklinkName(name: string): string {
      return this.backlink_renames[name] || `backlink_via_${ name }`
  }
}
...
class ModelGenerator {
    ...
    for (const [name, rec] of Object.entries(maps.object_types)) {
      ...
      for (const link of rec.links) {
        if (link.cardinality === 'One') {
          const lname = link.name
          const bklink = mod.getBacklinkName(lname)
    ...
    for (const [table, rec] of Object.entries(maps.link_objects)) {
      ...
      for (const prop of rec.links) {
        ...
        if (mtgt !== 'default') {
            ...
            warn(`Skipping link ${fwname}: link target ${ mtgt + "::" + tar } `
                 + `is not supported`)
            continue
        }
        if (prop.name === 'target') {
      ...
      const bkname = src.getBacklinkName(fwname)
      ...
      // ManyToManyField.
      src.mlinks[fwname] = (
        `${ mod.name }[]    @relation("${ bkname }")`
      )
      tgt.mlinks[bkname] = (
        `${ mod.name }[]    @relation("${ mod.name }")`
      )
  ...
  renderBacklinkLink(link: JSONLink): string {
      ...
      return (
        `${ target }${ multi }    @relation("${ link.name }")`
      )
  }
