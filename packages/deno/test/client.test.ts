...
import {Buffer, test, expect, jest} from "../_src/globals.deno.ts";

import {parseConnectArguments} from "../_src/conUtils.ts";
import {
  Client,
  DivisionByZeroError,
  Duration,
...
import {retryingConnect} from "../_src/retry.ts";
import {AdminUIFetchConnection} from "../_src/fetchConn.ts";
import {CustomCodecSpec} from "../_src/codecs/registry.ts";
import {
...
if (!isDeno && getAvailableFeatures().has("binary-over-http")) {
  test("binary protocol over http", async () => {
    //@ts-ignore
    const tokenFile = require("path").join(new URL('.', import.meta.url).pathname, "keys", "jwt");
    //@ts-ignore
